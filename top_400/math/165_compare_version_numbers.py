...
of a version number to be 0. For example, version 
number 3.4 has a revision number of 3 and 4 for its first 
...
both “01” and “001" represent the same number “1”
...
have a third level revision number, which means 
its third level revision number is default to "0"

Note:

Version strings are composed of numeric strings 
separated by dots . and this numeric strings may 
have leading zeroes.
Version strings do not start or end with dots, and 
they will not be two consecutive dots.
'''
...
class Solution(object):
    def compareVersion(self, version1, version2):
        ...
        v1l = version1.split(".")
        v2l = version2.split(".")
        num_bit = max(len(v1l), len(v2l))
        print(v1l, v2l, num_bit)
        v1, v2 = 0, 0
        for i in range(num_bit):
            v1 = v1 * 10 + (eval(v1l[i].lstrip("0")) 
                                    if i < len(v1l) and v1l[i].lstrip("0") != ""
                                    else 0)
            v2 = v2 * 10 + (eval(v2l[i].lstrip("0")) 
                                    if i < len(v2l) and v2l[i].lstrip("0") != ""
                                    else 0)
        if v1 > v2:
            return 1
        elif v1 == v2:
